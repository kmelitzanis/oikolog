...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <h1 class="text-2xl font-extrabold text-gray-900">Income</h1>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        @foreach([
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-1">
                ...
                <div class="text-2xl font-extrabold text-gray-900 mt-1">{{ $s['value'] }}</div>
                <div class="text-sm text-gray-400 font-medium">{{ $s['label'] }}</div>
            </div>
        @endforeach
    </div>
    ...
    <form method="GET" action="{{ route('income.index') }}" class="flex flex-wrap gap-3 mb-6" x-data>
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search income…"
               class="flex-1 min-w-40 bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition">
        <select name="frequency" @change="$el.form.submit()"
                class="bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-emerald-500 transition">
        ...
        @if(request()->hasAny(['search','frequency','status']))
            <a href="{{ route('income.index') }}"
               class="inline-flex items-center gap-1.5 bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-500 hover:bg-gray-50 transition">Clear</a>
        @endif
    </form>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        @forelse($incomes as $income)
            @php
                ...
                $rowClass  = $isOverdue ? 'bg-red-50' : ($isSoon ? 'bg-amber-50' : 'bg-white');
                $amtClass  = 'text-emerald-600 font-bold';
            @endphp
            <div
                class="flex items-center gap-3 sm:gap-4 px-4 py-4 {{ !$loop->last ? 'border-b border-gray-50' : '' }} {{ $rowClass }} hover:brightness-95 transition"
                x-data>
                ...
                <div class="flex-1 min-w-0 cursor-pointer"
                    ...
                    <div
                        class="text-xs mt-0.5 {{ $isOverdue ? 'text-red-500' : ($isSoon ? 'text-amber-500' : 'text-gray-400') }}">
                ...
                {{-- Frequency badge --}}
                <span
                    class="hidden md:inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700">
                ...
                @if(!$income->is_active)
                    <span
                        class="hidden md:inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-500">Inactive</span>
                @elseif($isOverdue)
                    <span
                        class="hidden md:inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-700">Late</span>
                @elseif($isSoon)
                    <span
                        class="hidden md:inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-700">Soon</span>
                @endif
                ...
                <div class="text-right shrink-0 cursor-pointer"
                    ...
                    <div class="text-xs text-gray-400">{{ number_format($income->monthlyEquivalent(), 2) }}/mo</div>
                </div>
                ...
                <div class="flex items-center gap-1.5 shrink-0" @click.stop>
                    ...
                    <form method="POST" action="{{ route('income.destroy', $income) }}">
                        ...
                        <button type="submit" title="Delete"
                                ...
                                class="w-8 h-8 flex items-center justify-center rounded-xl bg-red-50 text-red-400 hover:bg-red-100 transition">
        ...
        @empty
            <div class="flex flex-col items-center justify-center py-16 text-gray-400">
                ...
                <a href="{{ route('income.create') }}"
                   class="inline-flex items-center gap-2 bg-emerald-600 text-white text-sm font-semibold rounded-xl px-4 py-2.5 hover:bg-emerald-700 transition">
